...
- Go 1.24 + Uber fx for dependency injection, Google gRPC for service communication, grpc-gateway for REST API, MongoDB driver v2, MinIO client, JWT authentication, buf for protobuf generation, goverter for struct conversion, NATS for messaging (001-admin-kit-distribution)
- MongoDB for document storage, MinIO for file/object storage (001-admin-kit-distribution)
...
## Recent Changes
- 001-admin-kit-distribution: Added Go 1.24 + Uber fx for dependency injection, Google gRPC for service communication, grpc-gateway for REST API, MongoDB driver v2, MinIO client, JWT authentication, buf for protobuf generation, goverter for struct conversion, NATS for messaging
...
## Qwen Added Memories
- Всегда старайся следовать конституции проекта
